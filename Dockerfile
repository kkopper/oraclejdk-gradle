...
RUN wget -q --output-document=gradle-bin.zip https://services.gradle.org/distributions/gradle-4.8.1-bin.zip
RUN mkdir -p /opt/gradle
RUN unzip -q -d /opt/gradle gradle-bin.zip
RUN rm gradle-bin.zip

ENV JAVA_HOME "/jdk-10.0.1"
ENV PATH "${PATH}:${JAVA_HOME}/bin:/opt/gradle/gradle-4.8.1/bin"
