...
RUN wget -q --output-document=gradle-bin.zip https://services.gradle.org/distributions/gradle-4.5.1-bin.zip
RUN mkdir -p /opt/gradle
RUN unzip -q -d /opt/gradle gradle-bin.zip
RUN rm gradle-bin.zip

ENV JAVA_HOME "/jdk1.8.0_161"
ENV PATH "${PATH}:${JAVA_HOME}/bin:/opt/gradle/gradle-4.5.1/bin"
